...
import {
    ...
    Avatar,
    Box, Button,
    FlatList, FormControl,
    HamburgerIcon,
    Heading,
    HStack,
    Icon,
    IconButton, Input, Menu,
    Modal,
    Pressable,
    Spacer,
    Text,
...
import {useDeleteChatMutation, useRemoveMemberMutation, useUpdateChatMutation} from "../../services/chatService";
...
const ChatSettings: React.FC<ChatSettingsProps> = ({isOpen, onClose, chat}) => {
    ...
    return (
        <>
                ...
                <Modal.Content w={'90%'} maxW={'100%'}>
                    ...
                    <Modal.Body px={3} py={0}>
                        <Box flexDir={'row'} py={4} mb={4} alignItems={'center'} borderBottomWidth={1} borderBottomColor={'coolGray.100'}>
                            <Pressable position={'relative'} onPress={() => setImageDialogOpen(true)}>
                                ...
                                {chat.type === "group" &&
                                    <Icon as={Ionicons} name={'add'} position={'absolute'} bottom={-6} right={2} size={'xl'} color={'secondary.500'} borderRadius={999}  />}
                            </Pressable>
                            ...
                            <Menu w="190" trigger={(triggerProps: any) => {
                                ...
                                {user?.id === chat.admin.id || chat.type === "direct" ? (
                                    <Menu.Item onPress={() => openConfirmation('delete')} alignItems={'center'}>
                                        <Icon as={Ionicons} name={'trash'} size={'md'} />
                                        <Heading size={'md'}>Delete {chat.type}</Heading>
                                    </Menu.Item>
                                ) : (
                                    <Menu.Item onPress={() => openConfirmation('leave')} alignItems={'center'}>
                    ...
                    <FlatList w={'100%'} px={3} data={chat.members} renderItem={({item}) => (
                        <ChatMemberItem item={item}
                                        isAdmin={chat.admin.id === user?.id && item.user.id !== user?.id}
                                        onRemove={() => {
            ...
            <SettingAlert  open={openAlertDialog}
                ...
                <Modal.Content>
                    ...
                    <Modal.Body>
                        <FormControl>
                            ...
                            <Input value={image_url} onChangeText={(text) => setImage_url(text)}  />
                        </FormControl>
                    </Modal.Body>
                    <Modal.Footer>
                        <Button.Group space={2}>
                            <Button variant="ghost" colorScheme="blueGray" onPress={closeImageDialog}>
                                Cancel
                            </Button>
...
const SettingAlert: React.FC<SettingAlertProps> = ({title, open, setOpen, setConfirm}) => {
    ...
    return (
        <AlertDialog leastDestructiveRef={cancelRef} onClose={cancel} isOpen={open}>
            <AlertDialog.Content>
                ...
                <AlertDialog.Footer>
                    <Button.Group space={2}>
                        <Button variant="unstyled" colorScheme="coolGray" onPress={cancel} ref={cancelRef}>
                            Cancel
                        </Button>
